...
exports.renderSignup = function(req, res, next) {
	...
	if (!req.user) {
		...
		res.render('signup', {
			// Set the page title variable
			title: 'Sign-up Form',
			headerText: 'Bem vindo ao time! Cadastre-se no formulário abaixo:',
			// Set the flash message variable
...
// This will check whether a user is authenticated at all
...
// The list method is used to query all the users
...
exports.userByID = function(req, res, next, id) {
  User.findById(id)
    ...
    if (err) return next(err);
    if (!user) return next(new Error('Não foi possível carregar o usuario ' + id));
    req.user = user;
    next();
  });
};
